...
func TestRlpDecodeParentHash(t *testing.T) {
	...
	if rlpData, err := rlp.EncodeToBytes(&types.Header{ParentHash: want}); err != nil {
		t.Fatal(err)
	} else {
		if have := types.HeaderParentHashFromRLP(rlpData); have != want {
	...
	if rlpData, err := rlp.EncodeToBytes(&types.Header{
		ParentHash: want,
	...
	}
	{
		...
		for i, rlpData := range [][]byte{
			...
			if have, want := types.HeaderParentHashFromRLP(rlpData), (common.Hash{}); have != want {
