...
// 16进制字符串转[]byte,16进制字符串必须以0x开头并且是偶数位。`""`也是可以的。
func TestUnmarshalBytes(t *testing.T) {
	var unmarshalBytesTests = []unmarshalTest{
		// invalid encoding
		{input: "", wantErr: errors.New("unexpected end of JSON input")},
		{input: "null", wantErr: errors.New("uerrNonString(bytesT)")},
		{input: "10", wantErr: errors.New("uerrNonString(bytesT)")},
		{input: `"0"`, wantErr: errors.New("uwrapTypeError(ErrMissingPrefix, bytesT)")},
		{input: `"0x0"`, wantErr: errors.New("uwrapTypeError(ErrOddLength, bytesT)")},
		{input: `"0xxx"`, wantErr: errors.New("uwrapTypeError(ErrSyntax, bytesT)")},
		{input: `"0x01zz01"`, wantErr: errors.New("uwrapTypeError(ErrSyntax, bytesT)")},
...
func TestMarshalBytes(t *testing.T) {
	...
	for _, test := range encodeBytesTests {
		in := test.input.([]byte)
		out, err := json.Marshal(hexutil.Bytes(in))
		if err != nil {
			fmt.Println(test.input, "错误1")
			fmt.Println("----")
			t.Errorf("%x: %v", in, err)
		...
		if out := hexutil.Bytes(in).String(); out != test.want {
			fmt.Println(test.input, "错误3")
			fmt.Println("----")
