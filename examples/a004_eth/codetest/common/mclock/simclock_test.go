...
var _ mclock.Clock = mclock.System{}
var _ mclock.Clock = new(mclock.Simulated)
...
func TestSimulatedAfter(t *testing.T) {
	...
	select {
	case stamp := <-ch:
		want := mclock.AbsTime(0).Add(offset).Add(timeout)
		if stamp != want {
...
func TestSimulatedSleep(t *testing.T) {
	var (
		c       mclock.Simulated
		timeout = 1 * time.Hour
		done    = make(chan mclock.AbsTime, 1)
	)
	go func() {
		c.Sleep(timeout)
		done <- c.Now()
...
func TestSimulatedTimerReset(t *testing.T) {
	...
	select {
	case ftime := <-timer.C():
		if ftime != mclock.AbsTime(timeout) {
			t.Fatalf("wrong time %v sent on timer channel, want %v", ftime, mclock.AbsTime(timeout))
		}
	default:
		t.Fatal("timer didn't fire")
	}
	...
	select {
	case ftime := <-timer.C():
		if ftime != mclock.AbsTime(3*timeout) {
			t.Fatalf("wrong time %v sent on timer channel, want %v", ftime, mclock.AbsTime(3*timeout))
		}
	default:
